/**
 * E2E tests for draft task workflows
 * Tests the complete user journey: create draft -> edit -> edit -> start task
 */
...
describe("Draft Task E2E Workflow", () => {
  ...
  beforeAll(async () => {
    ...
    testDataDir = await mkdtemp(join(tmpdir(), "clanky-draft-e2e-test-data-"));
    testWorkDir = await mkdtemp(join(tmpdir(), "clanky-draft-e2e-test-work-"));
  ...
  test("create draft -> edit -> edit -> start task (immediate execution)", async () => {
    // Step 1: Create draft
    ...
    const firstEditResponse = await fetch(`${baseUrl}/api/tasks/${taskId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    const secondEditResponse = await fetch(`${baseUrl}/api/tasks/${taskId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    // Step 4: Start task (immediate execution)
    ...
    const finalResponse = await fetch(`${baseUrl}/api/tasks/${taskId}`);
    const finalBody = await finalResponse.json();
    expect(finalBody.config.prompt).toBe("Final task description");
    expect(finalBody.config.maxIterations).toBe(5);
    
    // Git branch should be created after task started
    // Note: In tests with mock backend, git setup may not complete if task finishes too quickly
    // Just verify the task completed successfully
    expect(finalBody.state.status).toBe("completed");
  });

  test("create draft -> edit -> edit -> start task (plan mode)", async () => {
    ...
    // Step 4: Start task in plan mode
    const startResponse = await fetch(`${baseUrl}/api/tasks/${taskId}/draft/start`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    // Verify task entered planning status
    ...
    // In mock test environment, git setup may not complete if task finishes too quickly
    // Just verify the task reached a valid state
  ...
  test("create draft -> do not edit -> start immediately", async () => {
    ...
    const uniqueWorkDir = await mkdtemp(join(tmpdir(), "clanky-draft-e2e-quick-"));
    ...
    try {
      ...
      await waitForTaskStatus(taskId, ["completed", "error", "stopped"], 30000);
    } finally {
      await rm(uniqueWorkDir, { recursive: true, force: true });
    }
  });

  test("cannot start non-draft task via draft/start endpoint", async () => {
    // Create regular (non-draft) task
    const createResponse = await fetch(`${baseUrl}/api/tasks`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  test("workspaceId is included when fetching draft task for editing", async () => {
    // This test ensures that when fetching a draft task (e.g., to edit it),
    ...
    // Step 2: Fetch the draft task (simulating opening the edit dialog)
    const fetchResponse = await fetch(`${baseUrl}/api/tasks/${taskId}`);
    ...
    const editResponse = await fetch(`${baseUrl}/api/tasks/${taskId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  test("plan mode checkbox can be unchecked and persists", async () => {
    ...
    const secondUpdateResponse = await fetch(`${baseUrl}/api/tasks/${taskId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  test("sequential draft edits preserve prompt correctly", async () => {
    ...
    const create1Response = await fetch(`${baseUrl}/api/tasks`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...baseCreateTaskPayload,
    ...
    const task1Id = create1Body.config.id;
    ...
    const create2Response = await fetch(`${baseUrl}/api/tasks`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...baseCreateTaskPayload,
    ...
    const task2Id = create2Body.config.id;
    ...
    const get1Response = await fetch(`${baseUrl}/api/tasks/${task1Id}`);
    ...
    const update1Response = await fetch(`${baseUrl}/api/tasks/${task1Id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    const get2Response = await fetch(`${baseUrl}/api/tasks/${task2Id}`);
    ...
    const verify1Response = await fetch(`${baseUrl}/api/tasks/${task1Id}`);
