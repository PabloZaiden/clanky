...
import { emitAutomaticPrFlowUpdatedEvent } from "../../src/core/task/task-automatic-pr-flow-events";
...
describe("emitAutomaticPrFlowUpdatedEvent", () => {
  test("emits the persisted automatic PR flow state for subscribers", () => {
    ...
    emitAutomaticPrFlowUpdatedEvent(emitter, "task-1", {
      enabled: true,
      status: "monitoring",
    ...
    expect(event?.type).toBe("task.automatic_pr_flow.updated");
    if (event?.type === "task.automatic_pr_flow.updated") {
