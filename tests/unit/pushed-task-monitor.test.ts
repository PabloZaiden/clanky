...
describe("PushedTaskMonitor", () => {
  ...
  test("auto-marks a pushed task as merged only once", async () => {
    const executor = new StubExecutor();
    const git = new StubGitService();
    const storedTask = createTaskWithStatus("pushed", {
      config: {
        directory: "/tmp/repo",
    ...
    const monitor = new PushedTaskMonitor({
      ...
      loadTask: async () => storedTask,
        updateTaskState: async (_taskId, state) => {
          storedTask.state = state;
          return true;
        },
        getCommandExecutor: async () => executor,
        ...
        storedTask.state = {
          ...storedTask.state,
          status: "merged",
          reviewMode: storedTask.state.reviewMode
            ? { ...storedTask.state.reviewMode, addressable: false }
            : undefined,
          };
          return { success: true };
        },
        probePullRequestMonitoring: async (task, directory, depExecutor, depGit) =>
          await import("../../src/core/pull-request-navigation").then((module) =>
            module.probePullRequestMonitoring(task, directory, depExecutor, depGit)
          ),
        ensureAutomaticPrFlowPullRequest: async () => {
          throw new Error("not used");
        },
    ...
    expect(storedTask.state.status).toBe("merged");
    expect(storedTask.state.pullRequestMonitoring).toEqual({
      status: "merged",
  ...
  test("tracks automatic PR flow state for enabled tasks", async () => {
    const executor = new StubExecutor();
    const storedTask = createTaskWithStatus("pushed", {
      config: {
        directory: "/tmp/repo",
    ...
    expect(storedTask.state.automaticPrFlow).toEqual({
      enabled: true,
      status: "monitoring",
      startedAt: "2026-04-11T04:00:00.000Z",
  ...
  test("updates a behind PR branch through updateBranch and records the request", async () => {
    const executor = new StubExecutor();
    const storedTask = createTaskWithStatus("pushed", {
      config: {
        directory: "/tmp/repo",
    ...
    const monitor = new PushedTaskMonitor({
      ...
      startAutomaticPrReviewCycle: async () => {
        reviewCycleStarts += 1;
        return { success: true, reviewCycle: 1 };
      },
      resolveAutomaticPrFlowReviewThread: async () => {},
      intervalMs: 60_000,
    });
    ...
    expect(updateBranchCalls).toBe(1);
    expect(reviewCycleStarts).toBe(0);
    expect(storedTask.state.pullRequestMonitoring).toEqual({
      status: "open",
      lastCheckedAt: expect.any(String),
      pullRequestNumber: 42,
  ...
  test("does not retrigger a behind PR update while a recent request is still pending", async () => {
    ...
    const storedTask = createTaskWithStatus("pushed", {
      ...
      state: {
        ...
        pullRequestMonitoring: {
          ...
          branchUpdate: {
            status: "requested",
            lastDetectedAt: "2026-04-11T04:05:00.000Z",
            lastTriggeredAt,
          },
        },
        automaticPrFlow: {
    ...
    const monitor = new PushedTaskMonitor({
      ...
      isTaskRunning: () => false,
      probePullRequestMonitoring: async () => storedTask.state.pullRequestMonitoring!,
      ensureAutomaticPrFlowPullRequest: async () => pullRequest,
      fetchAutomaticPrFlowSnapshot: async () => ({
        pullRequest,
        reviewThreads: [],
    ...
    expect(updateBranchCalls).toBe(0);
    expect(storedTask.state.pullRequestMonitoring?.branchUpdate).toEqual({
      status: "requested",
      lastDetectedAt: expect.any(String),
      lastTriggeredAt,
      lastError: undefined,
  ...
  test("records conflicts when automatic branch updates enter conflict resolution", async () => {
    ...
    expect(storedTask.state.status).toBe("resolving_conflicts");
    expect(storedTask.state.pullRequestMonitoring?.branchUpdate).toEqual({
      status: "conflicts",
      lastDetectedAt: expect.any(String),
      lastTriggeredAt: expect.any(String),
  ...
  test("records automatic branch update failures without stopping PR monitoring", async () => {
    const executor = new StubExecutor();
    const storedTask = createTaskWithStatus("pushed", {
      config: {
        directory: "/tmp/repo",
    ...
    expect(storedTask.state.automaticPrFlow?.status).toBe("monitoring");
    expect(storedTask.state.pullRequestMonitoring?.lastError).toBe("push denied");
    expect(storedTask.state.pullRequestMonitoring?.branchUpdate).toEqual({
      status: "failed",
      lastDetectedAt: expect.any(String),
      lastTriggeredAt: expect.any(String),
  ...
  test("normalizes malformed handled items before filtering actionable feedback", async () => {
    ...
    const automaticPrFlowState = storedTask.state.automaticPrFlow;
    ...
    const monitor = new PushedTaskMonitor({
      ...
      extractAutomaticPrFeedback: async (_task, _directory, feedbackItems) => ({
        feedbackItems: [{
    ...
    expect(storedTask.state.automaticPrFlow?.handledItems).toEqual([]);
  });
  ...
  test("persists automatic PR flow errors without breaking the scheduler", async () => {
    ...
    expect(storedTask.state.pullRequestMonitoring?.status).toBe("open");
  });
  ...
  test("starts one automatic review cycle when new actionable feedback appears", async () => {
    ...
    expect(startedBatches).toHaveLength(1);
    expect(startedBatches[0]?.itemIds).toEqual(["thread-1"]);
    expect(storedTask.state.automaticPrFlow?.status).toBe("processing_feedback");
    expect(storedTask.state.automaticPrFlow?.activeBatch?.itemIds).toEqual(["thread-1"]);
    expect(storedTask.state.automaticPrFlow?.activeBatch?.items).toEqual([
      { id: "thread-1", source: "review_thread", threadId: undefined },
    ]);
    expect(storedTask.state.automaticPrFlow?.activeBatch?.reviewCycle).toBe(1);
  });
  ...
  test("marks fully ignored automatic PR feedback as handled without starting a review cycle", async () => {
    ...
    expect(storedTask.state.automaticPrFlow?.status).toBe("monitoring");
    expect(storedTask.state.automaticPrFlow?.activeBatch).toBeUndefined();
    expect(storedTask.state.automaticPrFlow?.handledItems).toEqual([{
      id: "comment-1",
      source: "review_comment",
      outcome: "ignored",
  ...
  test("auto-pushes completed automatic review batches before resolving feedback", async () => {
    const executor = new StubExecutor();
    const storedTask = createTaskWithStatus("completed", {
      config: {
        directory: "/tmp/repo",
        workspaceId: "workspace-1",
      },
    ...
    expect(resolvedThreadIds).toEqual(["thread-1"]);
    expect(storedTask.state.status).toBe("pushed");
    expect(storedTask.state.automaticPrFlow?.status).toBe("monitoring");
    expect(storedTask.state.automaticPrFlow?.activeBatch).toBeUndefined();
    expect(storedTask.state.automaticPrFlow?.handledItems).toEqual([
      {
  ...
  test("keeps automatic review batches in finalizing feedback while push-triggered conflicts are being resolved", async () => {
    ...
    expect(storedTask.state.status).toBe("resolving_conflicts");
    expect(storedTask.state.automaticPrFlow?.status).toBe("finalizing_feedback");
    expect(storedTask.state.automaticPrFlow?.activeBatch?.batchId).toBe("batch-1");
    expect(storedTask.state.automaticPrFlow?.handledItems).toEqual([]);
  ...
  test("keeps automatic review batches active while the feedback task is still running", async () => {
    const executor = new StubExecutor();
    const storedTask = createTaskWithStatus("running", {
      config: {
        directory: "/tmp/repo",
        workspaceId: "workspace-1",
      },
  ...
  test("surfaces auto-push failures without marking feedback handled", async () => {
    ...
    expect(resolvedThreadIds).toEqual([]);
    expect(storedTask.state.automaticPrFlow?.status).toBe("error");
    expect(storedTask.state.automaticPrFlow?.lastError).toContain("push failed");
    expect(storedTask.state.automaticPrFlow?.activeBatch?.batchId).toBe("batch-1");
    expect(storedTask.state.automaticPrFlow?.handledItems).toEqual([]);
  });
});
