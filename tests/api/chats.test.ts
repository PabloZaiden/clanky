...
describe("Chats API Integration", () => {
  ...
  function installMockBackend(
    responses: string[],
    options: Pick<MockBackendOptions, "onSendPrompt" | "onSendPromptSettled"> = {},
  ): void {
  ...
  test("exports chat transcript as markdown without tool call details", async () => {
    ...
    expect(markdown).toContain("Please inspect the README");
    expect(markdown).toContain("I inspected it and found the issue.");
  ...
  test("cancels timed-out autogenerated chat names without persisting late output", async () => {
    const lifecycle: string[] = [];
    let signalNamePromptStarted!: () => void;
    const namePromptStarted = new Promise<void>((resolve) => {
      signalNamePromptStarted = resolve;
    });
    ...
    let signalAbortSettled!: () => void;
    const abortSettled = new Promise<void>((resolve) => {
      signalAbortSettled = resolve;
    });
    installMockBackend(["Assistant response"], {
      onSendPrompt: async () => {
        lifecycle.push("prompt-started");
        signalNamePromptStarted();
    ...
    const originalAbortSession = mockBackend.abortSession.bind(mockBackend);
    mockBackend.abortSession = async (sessionId) => {
      lifecycle.push("session-abort-started");
      await originalAbortSession(sessionId);
      lifecycle.push("session-abort-settled");
      signalAbortSettled();
    };
    chatManager.setNameGenerationTimeoutForTesting(0);
    ...
    try {
      ...
      const sendResponse = await fetch(`${baseUrl}/api/chats/${created.config.id}/messages`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: "This chat name request will time out" }),
      });
      expect(sendResponse.status).toBe(200);

      await namePromptStarted;
      const settled = await waitForChatIdle(created.config.id);
      expect(settled.state.messages.at(-1)).toMatchObject({
        role: "assistant",
      ...
      resolveLateResponse({
        ...
        content: "Late name must not be persisted",
        parts: [{ type: "text", text: "Late name must not be persisted" }],
      });
      await lateResponseSettled;
      const afterLateResponse = await loadChat(created.config.id);
      expect(afterLateResponse).not.toBeNull();
      if (!afterLateResponse) {
        throw new Error(`Chat ${created.config.id} disappeared after late response settlement`);
      }
      expect(afterLateResponse.config.name).toBe(originalName);
    } finally {
      chatManager.setNameGenerationTimeoutForTesting(undefined);
      installMockBackend(["Hello from chat API", "Second response"]);
    }
  });
