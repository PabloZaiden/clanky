...
describe("Chats API Integration", () => {
  ...
  test("cancels timed-out autogenerated chat names without persisting late output", async () => {
    ...
    mockBackend.sendPrompt = async (sessionId, prompt) => {
      ...
      if (!promptText.includes("Generate a short name for a chat")) {
        return originalSendPrompt(sessionId, prompt);
      }
    ...
    try {
      ...
      const afterLateResponse = await pollUntil(
        () => loadChat(created.config.id),
        (chat): chat is Chat => chat?.config.name === originalName,
        {
          description: `chat ${created.config.id} to retain its autogenerated name`,
          timeoutMs: 1000,
          formatLastObserved: (chat) => chat?.config.name ?? "not found",
        },
      );
