...
- Prefer structured `settings.sections[].rows` for settings; keep `render` only as an escape hatch for app-specific UI that cannot be expressed as rows.
...
- Do not duplicate the active route title immediately below the fixed framework title bar.
