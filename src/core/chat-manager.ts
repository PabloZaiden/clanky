...
function isGeneratedChatName(name: string, workspaceName: string): boolean {
  ...
  return generatedPattern.test(name.trim());
}
...
export class ChatManager {
  ...
  private async dispatchMessage(
    ...
    current = await this.renameAutogeneratedChatFromFirstMessage(current, backend, message);
  ...
  private async renameAutogeneratedChatFromFirstMessage(chat: Chat, backend: Backend, message: string): Promise<Chat> {
    if (!message) {
      return chat;
    }
    ...
    try {
      ...
      const updated = await this.updateChat(chat.config.id, { name });
      if (!updated) {
        return chat;
      }
      return {
  ...
  private async cleanupWorktree(chat: Chat): Promise<void> {
    ...
    const executor = await backendManager.getCommandExecutorAsync(chat.config.workspaceId, chat.config.directory);
    const git = GitService.withExecutor(executor);
    const managedWorktreePath = git.assertCanonicalManagedWorktreePath(chat.config.directory, chat.config.id, worktreePath);
