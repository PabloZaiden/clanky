...
export const ExecutionHostPreferredModelSchema = ModelConfigSchema.extend({
  providerID: z.enum(AGENT_PROVIDER_IDS),
});
...
export const ExecutionNodeConfigurationSchema = z.object({
  ...
  preferredModel: ExecutionHostPreferredModelSchema.nullable().default(null),
...
export const DiscoverExecutionHostAddressesRequestSchema = z.object({
  credentialToken: z.string().trim().min(1).nullable().optional(),
}).strict();
