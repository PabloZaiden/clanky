/**
 * Internal helpers for the tasks persistence layer.
...
/**
 * Convert a Task to a flat object for database insertion.
 */
 ...
 * This prevents a single corrupt row from crashing the entire listTasks() call.
 */
export function safeJsonParse<T>(json: string, fallback: T, fieldName: string, rowId: unknown): T {
  try {
    return JSON.parse(json);
  } catch (error) {
    log.warn(`Failed to parse JSON in field "${fieldName}" for task ${String(rowId)}: ${String(error)}`);
    return fallback;
  }
}
...
export function rowToTask(row: Record<string, unknown>): Task {
  ...
  const config: TaskConfig = {
    ...
    git: {
      ...
      commitScope: normalizeCommitScope(
        (row["git_commit_scope"] as string | null) ?? DEFAULT_TASK_CONFIG.git.commitScope,
      ) ?? "",
    },
    model,
  ...
  if (typeof row["cheap_model"] === "string") {
    const parsedCheapModel = safeJsonParse(
      ...
      DEFAULT_TASK_CONFIG.cheapModel,
      "cheap_model",
      row["id"],
    );
    ...
    if (validation.success) {
      ...
      log.warn(`Failed to validate cheap model selection for task ${String(row["id"])}`, {
