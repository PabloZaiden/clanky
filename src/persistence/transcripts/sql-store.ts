...
import {
  getTranscriptEntryKey as getEntryKey,
  getTranscriptRevision as getRevision,
  getTranscriptStateEntries,
  sortTranscriptStateEntries as sortStateEntries,
} from "./projection";
import { getTranscriptTableConfig as getTableConfig } from "./table-config";
import type {
...
function upsertEntry(
  ...
  const messageRole = entry.kind === "message" && isPersistedMessage(entry.payload)
    ? entry.payload.role
    : null;
...
export function hydrateTranscriptStateForUser(
  ...
  for (const entry of [...entries].sort((left, right) => left.sequence - right.sequence)) {
    if (entry.kind === "message") {
      if (isPersistedMessage(entry.payload)) {
        messages.push(entry.payload);
      }
      continue;
    }
    if (entry.kind === "log") {
      if (isTaskLogEntry(entry.payload)) {
