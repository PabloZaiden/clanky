...
/**
 * Folder-tree icon for workspace navigation.
 */
export function FolderTreeIcon({ className = "", size = "h-4 w-4" }: IconProps) {
  return (
    <svg
      className={`${size} ${className}`.trim()}
      fill="none"
...
/**
 * Server-rack icon for server navigation.
 */
export function ServerIcon({ className = "", size = "h-4 w-4" }: IconProps) {
  return (
    <svg
      className={`${size} ${className}`.trim()}
      fill="none"
