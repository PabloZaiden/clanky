...
export function PromptTab({ config, state, isActive }: PromptTabProps) {
  return (
    <div className={taskDetailsTabScrollContainerClassName}>
      <div className={`${taskDetailsTabContentFullWidthClassName} space-y-6`}>
        ...
        {state.pendingPrompt && (
          <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
            ...
            <p className="text-xs text-yellow-600 dark:text-yellow-400 mt-2">
              This message will be used the next time the task runs.
            </p>
          </div>
        )}
        ...
        {state.pendingModel && (
          <div className="bg-gray-50 dark:bg-neutral-900/40 border border-gray-200 dark:border-gray-800 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-700 dark:text-gray-300">
              Model will change to <span className="font-mono font-medium">{formatModelDisplay(state.pendingModel)}</span> the next time the task runs.
            </p>
          </div>
        )}
